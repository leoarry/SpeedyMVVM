...
namespace SpeedyMVVM.Utilities
{
    ...
    public class RelayCommand : IRelayCommand
    {
        ...
        public RelayCommand(Action handler, bool enable = false, string label = null, string iconPath = null)
        {
            _handler = handler;
            IsEnabled = enable;
            _Label = label;
        ...
        {
            return Task.Factory.StartNew(() => _handler());
        }
        ...
        public void CombineAction(Action newAction, bool newActionFirst = false)
        {
            if (newActionFirst)
                _handler = (Action)Delegate.Combine(newAction, _handler);
            else
                _handler = (Action)Delegate.Combine(_handler, newAction);
        }
